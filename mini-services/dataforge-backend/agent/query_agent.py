...
class QueryAgent:
    ...
    def __init__(self):
        ...
        # Lazily created — see _get_llm_agent(). Not created in __init__ to
        # avoid importing/instantiating the LLM client for callers that
        # never actually need SQL generation (e.g. schema-only utilities).
        self._llm_agent = None
        self._last_generation_meta = {"llm_used": False, "explanation": None}
...
9. Always use valid DuckDB SQL syntax"""
    ...
    def _find_column_by_keyword(self, question_lower: str, columns_info: Dict,
        ...
        # First, try exact column name match (word-boundary aware, so a
        # column like "age" doesn't false-match inside "average")
        for col_name in columns_info.keys():
            if re.search(rf"\b{re.escape(col_name.lower())}\b", question_lower):
                return col_name
    ...
    async def generate_sql(self, question: str, session_id: str = "default") -> str:
        """Generate SQL from natural language question.

        Tries the real LLM first (much better at handling filters, date
        ranges, comparisons, multi-condition questions, etc. than the
        keyword pattern-matcher). Falls back to the local heuristic matcher
        only if no LLM is configured or the LLM call fails.
        """
        self._refresh_schema()
        try:
            llm = self._get_llm_agent()
            result = await llm.generate_sql(question, self.schema, session_id=session_id)
            ...
            pass  # fall through to local heuristic below
    ...
    async def process_query(self, question: str) -> Dict[str, Any]:
        ...
        if "error" in result:
            return {
                ...
                "llm_used": self._last_generation_meta.get("llm_used", False),
            }
        ...
        return {
            ...
            "llm_used": self._last_generation_meta.get("llm_used", False),
            "sql_explanation": self._last_generation_meta.get("explanation"),
            "schema": {
