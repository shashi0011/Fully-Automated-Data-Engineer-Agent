...
class DuckDBTool:
    ...
    def _apply_string_op(series: pd.Series, op: str) -> pd.Series:
        ...
        elif op == "canonicalize_case":
            # Collapse casing/whitespace variants of the same category (e.g.
            # "New York" / "new york" / "NEW YORK ") into one consistent
            # value, so they aren't counted as separate categories in
            # group-bys, reports, or dedup.
            cleaned = s.loc[mask].astype(str).str.strip().str.replace(r"\s+", " ", regex=True)
            s.loc[mask] = cleaned.str.title()
    ...
    async def transform(self, user_id: str = None) -> Dict[str, Any]:
        ...
        for col in df.columns:
            ...
            if ops:
                for op in ops:
                    if op in {"trim", "lowercase", "normalize_empty_to_null", "canonicalize_case"}:
            ...
            if col in required_cols:
                # This column is required (id/name/person semantic) — leave
                # missing values as null instead of imputing them, so the
                # row-removal step below (which runs after this loop) can
                # actually find and drop them. Imputing first would silently
                # fabricate values (e.g. a fake customer name) and leave
                # nothing for the null-check to catch.
                logs.append(
                    {
                        "column": col,
                        "operation": "missing_value_imputation",
                        "strategy": "left_null_for_required_column_row_drop",
