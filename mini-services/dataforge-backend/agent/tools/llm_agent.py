...
class LLMAgent:
    ...
    async def generate_cleaning_plan(
        ...
        system_prompt = (
            ...
            "Use canonicalize_case (title-cases and collapses whitespace) for "
            "categorical/location/status-like text columns where the same value "
            "may appear with inconsistent casing (e.g. 'New York' / 'new york' / "
            "'NEW YORK') so they aren't treated as distinct categories.\n"
    ...
    @staticmethod
    def _best_match_column(question_lower: str, candidates: List[str]) -> Optional[str]:
        """Prefer a column whose name is explicitly mentioned in the question
        over just taking the first candidate in schema order. Uses word
        boundaries to avoid false positives like the column "age" matching
        inside the word "average"."""
    ...
    def _generate_fallback_sql(self, question: str, schema_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        best_numeric = self._best_match_column(question_lower, numeric_cols)
        best_category = self._best_match_column(question_lower, category_cols)

        sql = f"SELECT * FROM {table_name} LIMIT 20"
        explanation = "Show all records"

        if any(kw in question_lower for kw in ("top", "highest", "best")):
            if best_numeric and best_category:
                limit_match = re.search(r"top\s+(\d+)", question_lower)
                limit = int(limit_match.group(1)) if limit_match else 10
                sql = (
                    f"SELECT {best_category}, SUM({best_numeric}) as total "
                    f"FROM {table_name} GROUP BY {best_category} "
                    f"ORDER BY total DESC LIMIT {limit}"
                )
                explanation = f"Top {limit} {best_category} by total {best_numeric}"

        elif any(kw in question_lower for kw in ("average", "avg")):
            if best_numeric:
                if best_category and "by" in question_lower:
                    sql = (
                        f"SELECT {best_category}, AVG({best_numeric}) as avg_{best_numeric} "
                        f"FROM {table_name} GROUP BY {best_category} ORDER BY avg_{best_numeric} DESC"
                    )
                    explanation = f"Average {best_numeric} by {best_category}"
                else:
                    sql = f"SELECT AVG({best_numeric}) as avg_{best_numeric} FROM {table_name}"
                    explanation = f"Average {best_numeric}"

        elif any(kw in question_lower for kw in ("total", "sum")):
            if best_numeric:
                if best_category and "by" in question_lower:
                    sql = (
                        f"SELECT {best_category}, SUM({best_numeric}) as total_{best_numeric} "
                        f"FROM {table_name} GROUP BY {best_category} ORDER BY total_{best_numeric} DESC"
                    )
                    explanation = f"Total {best_numeric} by {best_category}"
                else:
                    sql = (
                        f"SELECT SUM({best_numeric}) as total_{best_numeric}, "
                        f"COUNT(*) as record_count FROM {table_name}"
                    )
                    explanation = f"Total {best_numeric} and record count"

        elif "by" in question_lower and best_category and best_numeric:
            sql = (
                f"SELECT {best_category}, SUM({best_numeric}) as total, "
                f"COUNT(*) as count FROM {table_name} "
                f"GROUP BY {best_category} ORDER BY total DESC"
            )
            explanation = f"Group by {best_category} with totals"
            ...
            if date_cols and best_numeric:
                sql = (
                    ...
                    f"SUM({best_numeric}) as total FROM {table_name} "
                    f"GROUP BY period ORDER BY period"
                )
                explanation = f"Monthly trend of {best_numeric}"

        elif any(kw in question_lower for kw in ("count", "how many")):
            if best_category:
                sql = (
                    f"SELECT {best_category}, COUNT(*) as count FROM {table_name} "
                    f"GROUP BY {best_category} ORDER BY count DESC"
                )
                explanation = f"Count by {best_category}"
            else:
        ...
        for col_name, col_info in columns.items():
            ...
            if semantic in ("money", "count", "score", "percentage") or any(
                ...
                if semantic == "email":
                    ...
                    "category", "location", "status", "gender",
                    "department", "region", "country",
                ):
                    ops.append("canonicalize_case")

            if semantic in ("id", "name", "person"):
