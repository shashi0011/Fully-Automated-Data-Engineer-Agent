...
def categorize_columns(columns: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    for col_name, col_info in columns.items():
        ...
        # ID columns — check both the semantic label already assigned by
        # schema_detector.py and common naming patterns. A bare column named
        # exactly "id" wasn't previously matched by "_id"/"id_"/"uuid"/
        # "identifier" (none of those substrings appear in "id" alone).
        if semantic == "id" or col_lower == "id" or any(
            kw in col_lower for kw in ["_id", "id_", "uuid", "identifier"]
        ):
        ...
        # Categorical columns
        elif any(kw in semantic for kw in [
            "category", "type", "status", "gender", "department",
            "region", "country", "name", "location", "city", "state",
