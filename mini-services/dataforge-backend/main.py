...
async def run_agent(request: CommandRequest):
    ...
    try:
        ...
        if any(token in cmd for token in ["full pipeline", "run pipeline", "run full", "etl pipeline", "execute pipeline"]):
            ...
            # Generate pipeline.py — previously this branch never called
            # PipelineGenerator at all, so a "run full pipeline" command
            # would ingest/transform/report but never actually produce a
            # pipeline.py file.
            try:
                from agent.pipeline_generator import PipelineGenerator
                pipeline_generator = PipelineGenerator()
                operations = ["ingest", "transform", "report"]
                pipeline_generator.generate(operations, schema)
                pipeline_filename = pipeline_generator._get_pipeline_filename(schema)
                pipeline_path = pipeline_generator._get_pipeline_path(schema)
                if os.path.exists(pipeline_path):
                    logs.append(f"[Pipeline] Written to pipelines/{pipeline_filename}")
                    rel_pipeline = _safe_relative(get_user_workspace(request.user_id)["root"], pipeline_path)
                    if rel_pipeline not in files:
                        files.append(rel_pipeline)
            except Exception as pipeline_exc:
                logs.append(f"[Pipeline] WARNING: pipeline.py generation failed: {pipeline_exc}")
