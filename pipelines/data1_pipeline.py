...
Generated at: 2026-04-09T13:32:38.975442
...
class DataPipeline:
    """Auto-generated Data Pipeline for medical dataset"""
    ...
    def __init__(self):
        ...
        source_file = os.path.normpath("R:\\New folder (2)\\my-project\\data\\raw\\data1.csv")
        if os.path.isabs(source_file):
            self.raw_data_path = source_file
        else:
            self.raw_data_path = os.path.normpath(os.path.join(self.base_path, source_file))
        self.clean_data_path = os.path.join(self.base_path, "data", "clean", "data1_clean.csv")
        self.report_path = os.path.join(self.base_path, "reports", "data1_clean_report.csv")
        self.logs = []
    ...
    def ingest(self):
        ...
        con.execute("CREATE OR REPLACE TABLE data1_raw AS SELECT * FROM df")
        
        count = con.execute("SELECT COUNT(*) FROM data1_raw").fetchone()[0]
        con.close()
    ...
    def transform(self):
        ...
        con.execute("""
            CREATE OR REPLACE TABLE data1_clean AS
            SELECT 
                patient_id,
                patient_name,
                ...
                CAST(treatment_cost AS INTEGER) as treatment_cost,
                recovery_status
            FROM data1_raw
        ...
        df = con.execute("SELECT * FROM data1_clean").fetchdf()
    ...
    def run(self):
        ...
        if results["transform"]:
            files.append("data/clean/data1_clean.csv") 
        self.log("Step 3: Generating report...")
        results["report"] = self.generate_report()
        if results["report"]:
            files.append("reports/data1_clean_report.csv")
