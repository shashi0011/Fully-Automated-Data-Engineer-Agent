...
Generated at: 2026-04-08T12:04:45.040669
...
class DataPipeline:
    ...
    def __init__(self):
        ...
        source_file = os.path.normpath("R:\\New folder (2)\\my-project\\data\\clean\\dirty_dataset_clean.csv")
        if os.path.isabs(source_file):
            self.raw_data_path = source_file
        else:
            self.raw_data_path = os.path.normpath(os.path.join(self.base_path, source_file))
        self.clean_data_path = os.path.join(self.base_path, "data", "clean", "dirty_dataset_clean_clean.csv")
        self.report_path = os.path.join(self.base_path, "reports", "dirty_dataset_clean_clean_report.csv")
        self.logs = []
    ...
    def ingest(self):
        ...
        con.execute("CREATE OR REPLACE TABLE dirty_dataset_clean_raw AS SELECT * FROM df")
        
        count = con.execute("SELECT COUNT(*) FROM dirty_dataset_clean_raw").fetchone()[0]
        con.close()
    ...
    def transform(self):
        ...
        con.execute("""
            CREATE OR REPLACE TABLE dirty_dataset_clean_clean AS
            SELECT 
                ...
                city,
                payment_method
            FROM dirty_dataset_clean_raw
            WHERE order_id IS NOT NULL
        """)
        
        os.makedirs(os.path.dirname(self.clean_data_path), exist_ok=True)
        df = con.execute("SELECT * FROM dirty_dataset_clean_clean").fetchdf()
    ...
    def generate_report(self):
        ...
        report_df = con.execute("""
            ...
            FROM dirty_dataset_clean_clean
            GROUP BY city
    ...
    def run(self):
        ...
        if results["transform"]:
            files.append("data/clean/dirty_dataset_clean_clean.csv") 
        self.log("Step 3: Generating report...")
        results["report"] = self.generate_report()
        if results["report"]:
            files.append("reports/dirty_dataset_clean_clean_report.csv")
