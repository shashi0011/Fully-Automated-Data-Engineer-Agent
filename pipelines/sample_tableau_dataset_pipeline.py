...
Generated at: 2026-04-08T17:16:53.971054
...
class DataPipeline:
    ...
    def __init__(self):
        ...
        source_file = os.path.normpath("R:\\New folder (2)\\my-project\\data\\raw\\sample_tableau_dataset_clean.csv")
        if os.path.isabs(source_file):
            self.raw_data_path = source_file
        else:
            self.raw_data_path = os.path.normpath(os.path.join(self.base_path, source_file))
        self.clean_data_path = os.path.join(self.base_path, "data", "clean", "sample_tableau_dataset_clean.csv")
        self.report_path = os.path.join(self.base_path, "reports", "sample_tableau_dataset_clean_report.csv")
        self.logs = []
    ...
    def ingest(self):
        ...
        con.execute("CREATE OR REPLACE TABLE sample_tableau_dataset_raw AS SELECT * FROM df")
        
        count = con.execute("SELECT COUNT(*) FROM sample_tableau_dataset_raw").fetchone()[0]
        con.close()
    ...
    def transform(self):
        ...
        con.execute("""
            CREATE OR REPLACE TABLE sample_tableau_dataset_clean AS
            SELECT 
                Category,
                Region,
                CAST(Sales AS INTEGER) as Sales,
                CAST(Profit AS INTEGER) as Profit,
                TRY_CAST(Date AS DATE) as Date
            FROM sample_tableau_dataset_raw
            WHERE 1=1
        """)
        
        os.makedirs(os.path.dirname(self.clean_data_path), exist_ok=True)
        df = con.execute("SELECT * FROM sample_tableau_dataset_clean").fetchdf()
    ...
    def generate_report(self):
        ...
        report_df = con.execute("""
            ...
            FROM sample_tableau_dataset_clean
            GROUP BY Category
    ...
    def run(self):
        ...
        if results["transform"]:
            files.append("data/clean/sample_tableau_dataset_clean.csv") 
        self.log("Step 3: Generating report...")
        results["report"] = self.generate_report()
        if results["report"]:
            files.append("reports/sample_tableau_dataset_clean_report.csv")
